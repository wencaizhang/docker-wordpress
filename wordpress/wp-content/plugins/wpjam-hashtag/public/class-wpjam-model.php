...
abstract class WPJAM_Model{
	...
	public static function move($id, $n){
		return static::get_handler()->move($id, $n);
	}
...
abstract class WPJAM_Item{
	...
	public function swap($id, $swap_id){
		if(intval($id) < intval($swap_id)){
			return self::move($id, 1);
		}else{
			return self::move($id, -1);
		}
	}

	public function move($id, $n=1){
		$items	= $this->get_items();
		...
		if(($i+$n) < 0 || ($i+$n) >= $max){
			return new WP_Error('over_limit', '超出边界，不可操作');
		}

		$swap_id	= $ids[$i+$n];
		
		$swap_item 			= $items[$swap_id];
		$items[$swap_id]	= $items[$id];
		$items[$id]			= $swap_item;
