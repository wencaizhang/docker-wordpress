	...
	<div class="author dynamic-intro-section">
		...
		<div class="hero-recent-post-list-wrapper w-dyn-list">
			<div class="hero-recent-post-list w-dyn-items">
				<div class="hero-recent-post-item w-dyn-item" style="background-image: url('<?php echo wpjam_theme_get_bg_img(); ?>');">
				</div>
			</div>
		</div>
	</div>
<?php } else{ $sticky_posts	= get_option('sticky_posts'); ?>
<div class="<?php echo ($sticky_posts && is_home() && !is_paged()) ? 'dynamic-intro-section' : 'dynamic-intro-section without-content'; ?>">
<?php if ( $sticky_posts && is_home() && !is_paged() ) {  $slide_query	= wpjam_query(['p' => $sticky_posts[0]]);  ?>
				...
				<div class="blog-list-wrapper w-dyn-list">
					<div class="blog-archive-list fullwidth w-clearfix w-dyn-items">
						<div class="blog-post-item w-dyn-item">
							<a class="blog-post-image-link-block w-inline-block" title="<?php the_title_attribute(); ?>" href="<?php the_permalink(); ?>" style="background-image: url('<?php echo wpjam_get_post_thumbnail_url(null, [1400,600]);?>');">
	...
	<div class="hero-recent-post-list-wrapper w-dyn-list">
		<div class="hero-recent-post-list w-dyn-items">
			<div class="hero-recent-post-item w-dyn-item" style="background-image: url('<?php global $post; echo wpjam_theme_get_bg_img($post); ?>');"></div>
		</div>
	</div>
...
<?php }else{ ?>
	...
	<div class="hero-recent-post-list-wrapper w-dyn-list">
		<div class="hero-recent-post-list w-dyn-items">
			<div class="hero-recent-post-item w-dyn-item" style="background-image: url('<?php echo wpjam_theme_get_bg_img(); ?>');"></div>
		</div>
	</div>
<?php } ?>
